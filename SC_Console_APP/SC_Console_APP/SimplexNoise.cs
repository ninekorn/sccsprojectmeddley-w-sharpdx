...
public class SimplexNoise
{
    ...
    public static float Noise(float xin, float yin)
    {
        ...
        float F2 = (float)(0.5f * (Math.Sqrt(3.0f) - 1.0f));
        float s = (xin + yin) * F2; // Hairy factor for 2D
        int i = fastfloor(xin + s);
        int j = fastfloor(yin + s);
        float G2 = (float)((3.0f - Math.Sqrt(3.0f)) / 6.0f);
        float t = (i + j) * G2;
    ...
    public static float SeamlessNoise(float x, float y, float dx, float dy, float xyOffset)
    {
        ...
        float nx = (float)(xyOffset + Math.Cos(s * 2.0f * Math.PI) * dx / (2.0f * Math.PI));
        float ny = (float)(xyOffset + Math.Cos(t * 2.0f * Math.PI) * dy / (2.0f * Math.PI));
        float nz = (float)(xyOffset + Math.Sin(s * 2.0f * Math.PI) * dx / (2.0f * Math.PI));
        float nw = (float)(xyOffset + Math.Sin(t * 2.0f * Math.PI) * dy / (2.0f * Math.PI));
